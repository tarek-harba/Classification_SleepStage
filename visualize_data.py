...
def plot_samples(data_path_dict : dict) -> None:
    ...
    for data_name, data_path in data_path_dict.items():
        ...
        axis[plt_nr].plot(sample_segment_values, label=f'Segment Label: {sample_segment_label}', linewidth=1.5)
    ...
    fig.text(0.04, 0.5, 'Channel-Recording Voltage', va='center', rotation='vertical', fontsize=12, fontweight='bold')

    fig.suptitle('Random 30-second Segments from each Dataset', fontsize=16, fontweight='bold')

    plt.tight_layout(rect=[0.05, 0.05, 1, 0.94])
    plt.show()

if __name__ == '__main__':
    data_path_dict = {'edf20': r'C:\Users\Doppler\PycharmProjects\ML_Paper_SleepClassification\Datasets\data_edf_20.npz',
